...
base_dir = r'C:\Users\ryand\Dropbox (EER)\Evolved Energy Research\Projects & Marketing\Australia\work files\renewable supply curves\v5'
input_directory = os.path.join(base_dir, 'formatted')
output_directory = os.path.join(base_dir, 'final curves')

atb_cf = pd.read_csv(os.path.join(base_dir, r'ATB_CAPACITY_FACTORS.csv'), index_col=None)
atb_capex = pd.read_csv(os.path.join(base_dir, r'ATB_CAPITAL_COSTS2.csv'), index_col=None)

formatted_cpa = pd.read_csv(os.path.join(input_directory, 'renewablesAU.csv'), index_col=False)
...
# # ### offshore wind
resource_df = formatted_cpa[formatted_cpa['file_name']=='offSupply_v6a_conserve70pct.csv'].copy()
...
resource_df = formatted_cpa[formatted_cpa['file_name']=='windSupply_v6a_conserve70pct.csv'].copy()
...
resource_df = formatted_cpa[formatted_cpa['file_name']=='pvSupply_v6a_conserve70pct.csv'].copy()
...
#
# resource_df = resource_df[resource_df['CF_wLL'] >= resource_df['tx_cost_per_kw']*M + b]
resource_df = resource_df[resource_df['tx_cost_per_kw']<350]
...
#
# resource_df = resource_df[resource_df['CF_wLL'] >= resource_df['tx_cost_per_kw']*M + b]
resource_df = resource_df[resource_df['tx_cost_per_kw']<200]

# features
feature_names =         ['CF_wLL', 'tx_cost_per_kw']
feature_weights = np.array([1.0,        0.1])
n_clusters = 3
...
capacity_all.to_csv(os.path.join(output_directory, 'capacity_constraints.csv'), index=False)
tx_cost_all.to_csv(os.path.join(output_directory, 'tx_cost.csv'), index=False)
capex_all.to_csv(os.path.join(output_directory, 'capex_cost.csv'), index=False)
depth_all.to_csv(os.path.join(output_directory, 'osw_depth.csv'), index=False)
capacity_factors_all.to_csv(os.path.join(output_directory, 'capacity_factors.csv'), index=False)
resource_df_all.to_csv(os.path.join(output_directory, 'binned_resource_df.csv'), index=False)