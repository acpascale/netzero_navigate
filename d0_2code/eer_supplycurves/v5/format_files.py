...
base_dir = r'D:\EER Dropbox\Ryan Jones\Evolved Energy Research\Projects & Marketing\Australia\work files\renewable supply curves\v5'
output_directory = os.path.join(base_dir, 'formatted')
input_directory = os.path.join(base_dir, 'raw inputs')

final_columns = ['file_name', 'OBJECTID', 'incap', 'CF_wLL', 'NZAu_maj', 'tx_cost_per_kw', 'depth_m', 'sensitivity', 'export', 'bin']


col_to_keep = ['OBJECTID', 'm_cf_noloss', 'incap', 'NZAu_maj', 'TXlength_km', 'spurLength_km', 'TXcost_maud2021', 'export', 'bin', 'm_elev']
col_renamed = ['OBJECTID', 'm_cf_noloss', 'incap', 'NZAu_maj', 'TXlength_km', 'spurLength_km', 'TXcost_maud2021', 'export', 'bin', 'depth_m']
...
for file_name in file_names:
    ...
    if file_name == 'pvSupply_v11_noSinkPopFilter_proRated_bulk330_proximityBased.csv':
...
resource_df.to_csv(os.path.join(output_directory, 'renewablesAU.csv'), index=False)
