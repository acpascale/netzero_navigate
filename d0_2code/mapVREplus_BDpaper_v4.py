# -*- coding: utf-8 -*-
"""
Created on Fri Jul 23 11:17:10 2021 -- last NZAU edit 30 August 2022, NZx 29/11/2023 -- last paper edit 14 July 2024

@author: ACP
"""
###This is the NZAu Biodiversity Paper distribution

###USE of this code requires proper attribution to the formative projects listed below as well as citation to this forthcoming paper:
## Pascale, A., Watson, J., Davis, D., Greig, C., Braer, M., Kiri, U., Smart, S. Addressing Climate and Biodiverity at scale and pace in Australia. In progress. (2024).
## --to be updated on publication

###CREDITS:
#1. The full process described in this code was seeded by the TNC's Power of Place California project (PoPC)
 # a) Wu, Grace C., Emily Leslie, Douglas Allen, Oluwafemi Sawyerr, D. Richard Cameron, Erica Brand, Brian Cohen, Marcela Ochoa, and Arne Olsen. “Power of Place, Land Conservation and Clean Energy Pathways for California,” 2019.
#2. ORIGINAL SOURCE CODE FOR STAGES 1-3 of the process was cloned from this gitHub Repo (https://github.com/cetlab-ucsb/mapre) on ~3 April 2021 for use on Net-Zero Australia. Projects prior to NZAu used toolbox versions of MapRE run in ArcMap.
 # a) Deshmukh, Ranjit, Grace Wu, and USDOE. “MapRE GIS Tools & Data (MapRE).” United States, March 10, 2016. https://doi.org/10.11578/dc.20210416.67.
 # b) Wu, Grace C., Ranjit Deshmukh, Kudakwashe Ndhlukula, Tijana Radojicic, Jessica Reilly-Moman, Amol Phadke, Daniel M. Kammen, and Duncan S. Callaway. “Strategic Siting and Regional Grid Interconnections Key to Low-Carbon Futures in African Countries.” Proceedings of the National Academy of Sciences 114, no. 15 (April 11, 2017): E3004–12. https://doi.org/10.1073/pnas.1611845114.
#3. The process was developed by the author, with collaborators listed as co-authors, on these projects
 # a) Larson, Eric, Chris Greig, Jesse Jenkins, Erin Mayfield, Andrew Pascale, Chuan Zhang, Joshua Drossman, et al. “Net-Zero America: Potential Pathways, Infrastructure, and Impacts.” Final Report. Princeton, NJ: Princeton University, October 29, 2021. https://netzeroamerica.princeton.edu/.
 #    - realizes working version of least-cost transmission fielding and costing process discussed as an area for further work by PoPC
 #    - for process details see Pascale, A, Jesse Jenkins. "Annex F: Integrated Transmission Line Mapping and Costing" (https://netzeroamerica.princeton.edu/img/NZA%20Annex%20F%20-%20HV%20Transmission.pdf)
 # b) Wu, Grace C., Ryan A. Jones, Emily Leslie, James H. Williams, Andrew Pascale, Erica Brand, Sophie S. Parker, et al. “Minimizing Habitat Conflicts in Meeting Net-Zero Energy Targets in the Western United States.” Proceedings of the National Academy of Sciences 120, no. 4 (January 24, 2023): e2204098120. https://doi.org/10.1073/pnas.2204098120.
 #    - extends methods developed for (a) to introduce more complex transmission routing surfaces, linking of mapped transmission costs to CPAs, and seperation of transmission siting process into two steps: a) routing step, b) costing step
 # c) Patankar, Neha, Xiili Sarkela-Basset, Greg Schivley, Emily Leslie, and Jesse Jenkins. “Land Use Trade-Offs in Decarbonization of Electricity Generation in the American West.” Energy and Climate Change 4 (December 1, 2023): 100107. https://doi.org/10.1016/j.egycc.2023.100107.
 #    - extends methods developed for (b) by allowing use of only a single 'costing' surface for all transmission line types (designed and manually implemented by A Pascale - see https://drive.google.com/file/d/1cdEjDRfhdcoogzjHrFdhA-vN1hpI9Ey1/view?usp=sharing)
 #    - prior methods and extension is coded into python/QGIS for use in REPEAT by REPEAT code team
 #    - REPEAT also developed new ways of combining CPAs and transmission and grouping them for use in capacity extension models (see journal paper)
 # d) Davis, Dominic, Andrew C. Pascale, Bishal Bharadaj, Richard Bolt, Michael Brear, Brendan Cullen, Robin Batterham, et al. “Methods, Assumptions, Scenarios & Sensitivities,” April 19, 2023. https://www.netzeroaustralia.net.au/wp-content/uploads/2023/04/Net-Zero-Australia-Methods-Assumptions-Scenarios-Sensitivities.pdf.
 #    - full process from one to seven is developed in python and arcPy and successfully implemented for use on Net Zero Australia project
 #       - MapRE code used in stages 1-3 was cloned from MapRE repo (see item 2)
 #       - Transmission steps previously run manually were coded into steps 4-7 and into the optional functions found in code for A1 to A6
 #    - full functionality of REPEAT project grouping was not possible to do on NZAu due to limits on time and computing infrastructure.
 #    - Code for stage 4 was provided to TNC Power of Place National project, and was used - with modification and - without attribution (as of 2 Feb 2024) to run TX routing for that project
#4. CODE CLEANED, REWRITTEN, AND PLACED IN SINGLE FILE FOR USE ON NZAu BIODIVERSITY PAPER (and to use in trainings)
 #    - deep clean on MapRE code, fixing bugs, updating functions, simplifying code, and rewriting to implement new functionality (e.g. heat maps and individual layer exports from stage 1)
 #       - improve memory handling; deleted (most) unused code; no longer supports CSP; rewrote to make exclusion layer overlay maps and simplify code; remove shapefile support for projects; added median and variety calculations;
 #       - removed all LCOE estimations; only allows capacity factors as input; Simplifies estimation of nameplate capacity and annual generation. Assumes that more complicated estimations of capacity, production, and LOOE will occur in later code (not included in this file)
 #    - provided as is to UoM Boundless project prior to publication of paper (April 2024).
 #    -(v4) updated file naming and transferred later stages from R code to python, stages 9 - 12 (July 2024)
#5. Bug discovery and fixes supplied by Yimin Zhang (UoM) who has prepared a fork of this code (and all processes after Stage 8 for use by UoM/UQ)
 #     - Step 9, fixes in sink transmission costing and in availability of offshore wind projects (these bugs impact NZAu results)
 #     - Option A0 - it was found that transmisison layers had left out exclusions of airports (layer j) in all transmission cost surfaces (no impact on NZAu results)

###NOTE ABOUT COMMENTS IN CODE
#d# Herafter double crosshatches will be USED to indicate in-line DEBUG code, commented out for normal runs
...
mastercsv_ = pd.read_csv( scriptpath + "\\mapVREplus\\" + "setup.csv" , header=None)
...
for column in mastercsv:
    ...
    B1Output       = OutputGDB + "\\_" + prefix + "B1" + "_case" + exCase
    ifTrue         = 1
    ifFalse        = 0
    # STAGE 2 variables #
    ...
    FishnetDir     = RootDir
    B2Output       = OutputGDB + "\\_" + prefix + "B2" + "_case" + exCase
    # STAGE 3 variables #
    Stage3CSVInput = scriptpath + data[29]
    B3Output       = OutputGDB + "\\_" + prefix + "B3"  + "_case" + exCase
    ...
    B4Output       = OutputGDB + "\\_" + prefix + "B4"  + "_case" + exCase
    routeBuf       = data[44]
    ...
    B5Output       = OutputGDB + "\\_" + prefix + "B5"  + "_case" + exCase
    bulkField      = data[49]
    ...
    B6Output       = OutputGDB + "\\_" + prefix + "B6"  + "_case" + exCase
    # STAGE 7 variables - Create spur TX Lines from cpa to nearest export node #
    B7Output       = OutputGDB + "\\_" + prefix + "B7"  + "_case" + exCase
    # STAGE 8 variables - Combine all infrastrcture to project#
    B8Output       = OutputGDB + "\\_" + prefix + "B8"  + "_case" + exCase
    ...
    WriteMsg( "MapRE+ Setup done" )
    WriteMsg("Base directory: " + baseDir)
    ...
    arcpy.env.workspace = ScratchGDB  # B1Output
    ...
    if startStage <= 1 and endStage >= 1:
        WriteMsg("B1:Starting B1 ")
        start_time = time.time()

        WriteMsg( "B1:Running " + Resource + " case " + exCase + ": process Layers = " + str(redoLayers) + ", heatmaps = " + str(heatmaps)  )
        ...
        if redoLayers == 1:
            for i,constraint in enumerate(inputDataPath):
                if inputDataPath[constraint][0] == "yes":
                    WriteMsg( "B1:Starting raster calculation for " + constraint)
                    cntRas += 1
                    outSav = OutputGDB + "\\ext" + str(i) + "_" + constraint
        ...
        if heatmaps == 1:
            ...
            WriteMsg("B1:Finished raster calculator for exclusion heatmaps")
            arcpy.management.CopyRaster( ras, OutputGDB + "\\exclusionHeatMap_ras_" + Resource + "_case" + exCase + "_layers" + str(cntRas))
            arcpy.management.Delete(ras)
            WriteMsg("B1:Finished exclusion raster heatmap")
            ras = Con( OutputGDB + "\\exclusionHeatMap_ras_" + Resource + "_case" + exCase + "_layers" + str(cntRas), ifTrue, ifFalse, "Value>0")
            arcpy.management.CopyRaster(ras, OutputGDB + "\\exclusionFlat_ras_" + Resource + "_case" + exCase + "_layers" + str(cntRas))
            tab = ZonalStatisticsAsTable(regAUS, "ISO_SUB", OutputGDB + "\\exclusionFlat_ras_" + Resource + "_case" + exCase + "_layers" + str(cntRas), ScratchGDB + "\\tab", "DATA", "SUM")
            ...
            arcpy.management.Delete(csvOUT + "schema.ini")
            arcpy.management.Delete(tab)
            WriteMsg("B1:Finished exclusion raster flatmap")
            ##generate B1 output
            ras = OutputGDB + "\\exclusionFlat_ras_" + Resource + "_case" + exCase + "_layers" + str(cntRas)
            rass = SetNull(ras, 1, "Value=1")
            #arcpy.management.CopyRaster( rass, B1Output + "_ras")
            arcpy.conversion.RasterToPolygon( rass , B1Output , "NO_SIMPLIFY" , "Value" , "MULTIPLE_OUTER_PART" )
            #arcpy.conversion.RasterToPolygon( B1Output + "_ras", B1Output  , "NO_SIMPLIFY", "Value", "MULTIPLE_OUTER_PART")
            #arcpy.management.Delete( ras )
            arcpy.management.Delete( rass )
            #arcpy.management.Delete(B1Output + "_ras")   ##raster is not used for the rest of teh processes, not saving and deleting the raster
            WriteMsg( "B1: Finished flattened B1 output")
        if heatmaps == 0:
            WriteMsg("B1: If any layers were redone, or heatmaps != 1, Note that no (new) B1 output will be created - Stage 1 must be run again with heatmaps == 1")

        #delete memory at end of stage
        arcpy.management.Delete("memory")
        end_time = time.time()
        WriteMsg("B1:Memory cache deleted, END mapRE+ B1 taking " + str ( round ( (end_time - start_time)/60 , 1 ) ) + " minutes.")
    ...
    if startStage <= 2 and endStage >= 2:
        WriteMsg("B2:Starting B2 with " + B1Output )
        start_time = time.time()
        ...
        if not (arcpy.Exists(Fishnet)):
            ...
            WriteMsg("B2:Creating fishnet " + fishnetSizeStr + " km in size to memory")
            ...
            WriteMsg("B2:Fishnet Generated")
            arcpy.conversion.ExportFeatures(fish, Fishnet)
            WriteMsg("B2:Creating template extent fishnet " + fishnetSizeStr  + " km in size to file: " + Fishnet)

        fish = arcpy.conversion.ExportFeatures(Fishnet, r"memory\fishnet_" + fishnetSizeStr + "km")
        WriteMsg("B2:Copying fishnet to memory :" + Fishnet)

        # COPY SUITABLE SITES FEATURE CLASS TO MEMORY
        sites = arcpy.conversion.ExportFeatures(B1Output, r"memory\suitableSites")
        WriteMsg("B2:Suitable sites in memory :" + B1Output)
        ...
        if Resource != "off":
            ...
            WriteMsg("B2:Finished intersecting total site area with fishnet and clipping to country bounds")
        else:
            fishclip = arcpy.analysis.Erase(fished, CountryBounds, ScratchGDB + "\\fishclip")
            WriteMsg("B2:Finished intersecting total site area with fishnet and clipping to offshore")

        #intermediate dump of memory in preperation for final processing steps
        arcpy.management.Delete("memory")
        WriteMsg("B2:Intermediate dump of memory cache")
        ...
        WriteMsg("B2:Sites Multipart to single")
        ...
        WriteMsg("B2:Sites part A: remove all singlepart sites below minimum contiguous field threshold")
        ...
        #arcpy.management.Merge( [ScratchGDB + "\\fishclipA", ScratchGDB + "\\fishclipD"] , B2Output )
        #arcpy.management.Delete(ScratchGDB + "\\fishclipA")
        #arcpy.management.Delete(ScratchGDB + "\\fishclipD")
        #WriteMsg("B2:B2 Output can be found at:" + B2Output)

        #d#TEMP section until the above works
        arcpy.management.Delete(fishclipSS)
        arcpy.conversion.ExportFeatures( ScratchGDB + "\\fishclipA" , B2Output )
        arcpy.management.Delete(ScratchGDB + "\\fishclipA")
        WriteMsg("B2:B2 Output can be found at:" + B2Output)
        ...
        arcpy.management.Delete("memory")
        end_time = time.time()
        WriteMsg("B2:Memory cache deleted, END mapRE+ B2 taking " + str ( round ( (end_time - start_time)/60 , 1 ) ) + " minutes.")
    ...
    if startStage <= 3 and endStage >= 3:
        WriteMsg("B3:Starting B3 with " + B2Output )
        start_time = time.time()

        #COPY SUITABLE SITES FEATURE CLASS TO MEMORY AND TO RASTER for use in analyses
        projects = arcpy.conversion.ExportFeatures(B2Output, r"memory\suitableSites")
        projects_ras = arcpy.conversion.PolygonToRaster( B2Output, "OBJECTID" , r"memory\projects_ras" )
        WriteMsg("B3:Suitable sites in memory; suitable site raster in memory for analyses with OBJECTID saved as raster value")
        ...
        WriteMsg("B3:Attribute list read from " + Stage3CSVInput)
        ...
        #save B3output and csvs for key datasets
        arcpy.conversion.ExportFeatures(projects, B3Output)
        arcpy.management.CopyRows(projects, ScratchGDB + "\\" + prefix + "B3_case" + exCase )
        arcpy.conversion.ExportTable(ScratchGDB + "\\" + prefix + "B3_case" + exCase , csvOUT + prefix + "B3_case" + exCase + "_cpa" + ".csv" )
        arcpy.management.Delete(csvOUT + prefix + "B3_case" + exCase + "_cpa" + ".csv.xml")
        arcpy.management.Delete(csvOUT + "schema.ini")
        #arcpy.management.Delete(ScratchGDB + "\\" + prefix + "B3_case" + exCase )
        WriteMsg("B3:Initial B3 Site Output can be found at:" + B3Output)
        WriteMsg("B3:CSV for initial B3 site Output can be found at:" + csvOUT + prefix + "B3_case" + exCase + "_cpa.csv")

        #delete memory at end of stage
        arcpy.management.Delete("memory")
        end_time = time.time()
        WriteMsg("B3:Memory cache deleted, END mapRE+ B3 taking " + str ( round ( (end_time - start_time)/60 , 1 ) ) + " minutes.")
    ...
    if redoSubs == 1:
        ...
        B3W = arcpy.conversion.ExportFeatures(( W , r"memory\projectsW" ))  ## memory\
        if arcpy.management.GetCount(B3W)[0] != "0":
            WriteMsg( "OptionA2: Wind projects points found - " + W  )
        B3O = arcpy.conversion.ExportFeatures(( O , r"memory\projectsO" ))  ## memory\
        if arcpy.management.GetCount(B3O)[0] != "0":
            WriteMsg( "OptionA2: Offshore Wind projects points found - " + O )
        B3P = arcpy.conversion.ExportFeatures(( P , r"memory\projectsP" ))  ## memory\
        if arcpy.management.GetCount(B3P)[0] != "0":
        ...
        BSA = arcpy.management.Merge([B3W, B3O, B3P], r"memory\projectsA")
        arcpy.management.Delete([B3W, B3O, B3P])
        ...
        arcpy.management.Delete("memory")
        end_time = time.time()
        WriteMsg("Option A2: Memory cache deleted, END mapRE+ Option A2 taking " + str ( round ( (end_time - start_time)/60 , 1 ) ) + " minutes.")
    ...
    if redoSurf_spur == 1:
        ...
        arcpy.management.Delete("memory")
        end_time = time.time()
        WriteMsg("Option A3: Memory cache deleted, END mapRE+ Option A3 taking " + str ( round ( (end_time - start_time)/60 , 1 ) ) + " minutes.")
    ...
    if redoSurf_bulk == 1:
        ...
        arcpy.management.Delete("memory")
        end_time = time.time()
        WriteMsg("Option A4: Memory cache deleted, END mapRE+ Option A4 taking " + str ( round ( (end_time - start_time)/60 , 1 ) ) + " minutes.")
    ...
    if redoSurf_sink == 1:
        ...
        arcpy.management.Delete("memory")
        end_time = time.time()
        WriteMsg("Option A5: Memory cache deleted, END mapRE+ Option A5 taking " + str ( round ( (end_time - start_time)/60 , 1 ) ) + " minutes.")
    ...
    if redoSurf_exp == 1:
        ...
        arcpy.management.Delete("memory")
        end_time = time.time()
        WriteMsg("Option A6: Memory cache deleted, END mapRE+ Option A6 taking " + str ( round ( (end_time - start_time)/60 , 1 ) ) + " minutes.")
    ...
    if startStage <= 4 and endStage >= 4:
        arcpy.management.Delete("memory")
        WriteMsg( "B4:Starting B4 with " + B3Output )
        start_time = time.time()
        ...
        if destTX == (gispath + "NZAu0_Base.gdb\GA_2021_Foundation_TXsubstations_3577"):
            WriteMsg("B4: Using GA dataset of existing substations, added subOID and removed all unneeded columns")
            WriteMsg("B4: this will fail during supply curve generation, unless a few fields are added in this script -- see A2 Option ('Sub2SA2_m' , 'subSA2aggOID' , 'subSA2aggPop' )" )
            ...
            WriteMsg("B4: Using pre-procesed generalized substations, removed all unneeded columns")
            arcpy.management.DeleteField(subs, ["Shape" , "subOID" , "sub2SA2_m" , "nzau" , "subSA2aggOID" , "subSA2aggPOP"], "KEEP_FIELDS")
        else:
            WriteMsg("B4: Using unknown destination layer for spur generation, if the attribute 'subOID' is not unique for every row or is not part of layer, then this or following processes will fail")
            WriteMsg("B4: In addition, use of this substation dataset will fail during supply curve generation, unless a few fields are added in this script -- see A2 Option ('Sub2SA2_m' , 'subSA2aggOID' , 'subSA2aggPop' )")

        #Get projects for transmission, clean attributes, and make points
        projects = arcpy.conversion.ExportFeatures( B3Output , r"memory\projects")
        ...
        WriteMsg("B4: Pairwise Buffer for constrained cost surface done")
        ...
        WriteMsg("B4: Extract by Mask for constrained cost surface done")
        ...
        WriteMsg("B4: Generated constrained cost surface for entire spur")
        ...
        arcpy.conversion.ExportFeatures( spursW , B4Output + "_spurs" )
        WriteMsg("B4: Spur line paths with costs generated and assigned closest substation")
        ...
        #######
        arcpy.conversion.ExportFeatures( subsW, B4Output + "_subs" )
        WriteMsg("B4: Exported Subststaions - Only those accessed")

        #save csvs for key datasets
        arcpy.management.CopyRows( B4Output + "_points" , csvOUT + prefix + "B3_case" + exCase + "_points.csv" )
        arcpy.management.CopyRows( B4Output + "_subs"   , csvOUT + prefix + "B3_case" + exCase + "_subs.csv" )  ##this can be removed once substation script is added!? or after new downscaling trialed
        arcpy.management.CopyRows( B4Output + "_spurs"  , csvOUT + prefix + "B3_case" + exCase + "_spurs.csv" )
        arcpy.management.Delete( csvOUT + prefix + "B3_case" + exCase + "_points.csv.xml")
        arcpy.management.Delete( csvOUT + prefix + "B3_case" + exCase + "_subs.csv.xml")
        arcpy.management.Delete( csvOUT + prefix + "B3_case" + exCase + "_spurs.csv.xml")
        arcpy.management.Delete( csvOUT + "schema.ini")
        WriteMsg("B4: Site CentrePoints can be found at:" + B4Output + "_points" )
        WriteMsg("B4: Accessed substations can be found at:" + B4Output + "_subs" )
        WriteMsg("B4: Spur lines can be found at:" + B4Output + "_spurs" )
        WriteMsg("B4: Saved csvs ( _points , _spurs , _subs )")

        #landfall for offshore
        if Resource == "off":
            WriteMsg("B4: Generating constrained cost surface for landfall")
            ...
            arcpy.conversion.ExportFeatures(  landfall , B4Output + "_landfall" )
            WriteMsg("B4: Landfall routes for offshore wind can be found at:" + B4Output + "_landfall")
            WriteMsg("B4: Landfall costs done , saving to _land csv")
            #save csvs for key datasets
            arcpy.management.CopyRows( B4Output + "_landfall" , csvOUT + prefix + "B4_case" + exCase + "_land.csv" )
            arcpy.management.Delete( csvOUT + prefix + "B4_case" + exCase + "_land.csv.xml")
            arcpy.management.Delete(csvOUT + "schema.ini")

        #delete memory at end of stage
        arcpy.management.Delete("memory")
        end_time = time.time()
        WriteMsg("B4: Memory cache deleted, END mapRE+ B4 taking " + str ( round ( (end_time - start_time)/60 , 1 ) ) + " minutes.")
    ...
    if startStage <= 5 and endStage >= 5:
        WriteMsg("B5: Starting B5 with " + B4Output )
        start_time = time.time()

        #load projects for transmission
        B4points = arcpy.conversion.ExportFeatures(  B4Output + "_points" , r"memory\projects" )
        #Spur lines from B4
        B4spurs = arcpy.conversion.ExportFeatures( B4Output + "_spurs" , r"memory\spurs" )
        #Selected substation locations
        sub = arcpy.conversion.ExportFeatures( B4Output + "_subs" , r"memory\sub" )
        ...
        WriteMsg("B5: Bulk line routes done")
        #Surface for cost
        ...
        WriteMsg("B5: Pairwise Buffer for bulk route constrained cost surface done")
        constCost = ExtractByMask( costSurf, ScratchGDB + "\\bulkRoutesBuf" )
        WriteMsg("B5: Extract by Mask for bulk route constrained cost surface done")
        ...
        WriteMsg("B5: Generated constrained cost surface for bulk lengths")
        ...
        WriteMsg("B5: Bulk line costs done")
        ...
        arcpy.conversion.ExportFeatures( bulkW , B5Output + "_bulk" )
        WriteMsg("B5: Spatial joins: SA2 to bulk lines")

        arcpy.management.CopyRows( B5Output + "_bulk" , csvOUT + prefix + "B5_case" + exCase + "_bulk.csv" )
        arcpy.management.Delete( csvOUT + prefix + "B5_case" + exCase + "_bulk.csv.xml")
        arcpy.management.Delete( csvOUT + "schema.ini")
        WriteMsg("B5: Bulk transmission lines from substations to nearest load can be found at:" + B5Output + "_bulk")
        WriteMsg("B5: Saved csvs ( _bulk )" )

        # delete memory at end of stage
        arcpy.management.Delete("memory")
        end_time = time.time()
        WriteMsg("B5: Memory cache deleted, END mapRE+ B5 taking " + str(round((end_time - start_time) / 60, 1)) + " minutes.")
    ...
    if startStage <= 6 and endStage >= 6:
        WriteMsg("B6:Starting B6 with " + B4Output )
        start_time = time.time()

        #Load substations for transmission
        sub = arcpy.conversion.ExportFeatures( B4Output + "_subs" , r"memory\projects")
        subs = arcpy.management.AddXY(sub)
        ...
        WriteMsg("B6: Sink line routes done")
        #Surface for cost
        Buf = arcpy.analysis.PairwiseBuffer (routes, r"memory\Buf", routeBuf )
        arcpy.conversion.ExportFeatures( Buf,  ScratchGDB + "\\sinkRoutesBuf" )
        WriteMsg("B6: Pairwise Buffer for sink route constrained cost surface done")
        constCost = ExtractByMask( costSurf, ScratchGDB + "\\sinkRoutesBuf" )
        WriteMsg("B6: Extract by Mask for sink route constrained cost surface done")
        ...
        WriteMsg("B6: Generated constrained cost surface for sink lengths")
        ...
        WriteMsg("B6: Spatial joins: SA2 to sink lines")

        #save sinks
        arcpy.conversion.ExportFeatures( sinkW, B6Output + "_sink" )
        arcpy.management.CopyRows( B6Output + "_sink" , csvOUT + prefix + "B6_case" + exCase + "_sink.csv" )
        arcpy.management.Delete( csvOUT + prefix + "B6_case" + exCase + "_sink.csv.xml")
        arcpy.management.Delete(csvOUT + "schema.ini")
        WriteMsg("B6: Sink transmission lines from substations to nearest regional sink load can be found at:" + B6Output + "_sink")
        WriteMsg("B6: Save csvs ( _sink )" )

        #delete memory at end of stage
        arcpy.management.Delete("memory")
        end_time = time.time()
        WriteMsg("B6: Memory cache deleted, END mapRE+ B6 taking " + str(round((end_time - start_time) / 60, 1)) + " minutes.")
    ...
    if startStage <= 7 and endStage >= 7:
        WriteMsg("B7:Starting B7 with " + B3Output )
        start_time = time.time()
        ...
        if Resource == "off":
            ...
            projectPts = arcpy.conversion.ExportFeatures(B4Output + "_points", r"memory\projects" , whereA)
            WriteMsg("B7: Offshore wind limited to " + str(arcpy.GetCount_management(projectPts)) + " projects.")
            exportDis = gdbTX + "\\exportOffDis"
            exportDir = gdbTX + "\\exportOffDir"
        elif Resource == "wind":
            whereA = "\"d_nodeOn\" <= " + str(250)
            projectPts = arcpy.conversion.ExportFeatures(B4Output + "_points", r"memory\projects" , whereA)
            WriteMsg("B7: Wind limited to " + str(arcpy.GetCount_management(projectPts)) + " projects.")
            exportDis = gdbTX + "\\exportOnDis"
            exportDir = gdbTX + "\\exportOnDir"
        else:
            whereA = "\"d_nodeOn\" <= " + str(250)
            projectPts = arcpy.conversion.ExportFeatures(B4Output + "_points", r"memory\projects" , whereA)
            WriteMsg("B7: Solar PV limited to " + str(arcpy.GetCount_management(projectPts)) + " projects.")
            exportDis = gdbTX + "\\exportOnDis"
            exportDir = gdbTX + "\\exportOnDir"
        ...
        WriteMsg("B7: Export line routes done")
        #Surface for cost
        spursBuf = arcpy.analysis.PairwiseBuffer( routes, r"memory\spursBuf", routeBuf )
        arcpy.conversion.ExportFeatures( spursBuf, ScratchGDB + "\\exportRoutesBuf" )
        WriteMsg("B7: Pairwise Buffer for constrained cost surface done")
        constCost = ExtractByMask( costSurf, ScratchGDB + "\\exportRoutesBuf" )
        WriteMsg("B7: Extract by Mask for constrained cost surface done")
        ...
        WriteMsg("B7: Generated constrained cost surface for entire export line")
        ...
        WriteMsg("B7: Export line costs done")
        ...
        arcpy.conversion.ExportFeatures( spursXX, B7Output + "_export" )
        WriteMsg("B7: Export spur transmission lines from project to nearest hydrogen production load can be found at:" + B7Output + "_export")
        WriteMsg("B7: Export spurs - Spatial joins: Export node info")

        ##landfall for offshore
        if Resource == "off":
            WriteMsg("B7: Generating constrained cost surface for landfall")
            #Surface for landfall
            DistAcc = DistanceAccumulation (destLand, in_cost_raster=constCost, in_surface_raster=elevSurf, out_back_direction_raster=r"memory\SROdir_temp")
            DistAcc.save(r"memory\SROdis_temp")
            landfall = OptimalPathAsLine( projectPts, r"memory\SROdis_temp", r"memory\SROdir_temp", r"memory\landfall", routeField, "EACH_CELL")
            arcpy.conversion.ExportFeatures( landfall, B7Output + "_exportLandfall" )
            WriteMsg("B7: Export spur transmission line landfall distances from offshore wind project to landfall point can be found at:" + B7Output + "_exportLandfall")
            arcpy.management.CopyRows( B7Output + "_exportLandfall" , csvOUT + prefix + "B7_case" + exCase + "_land.csv" )
            arcpy.management.Delete( csvOUT + prefix + "B7_case" + exCase + "_land.csv.xml")
            arcpy.management.Delete( csvOUT + "schema.ini")
            WriteMsg("B7: Landfall costs done and _land csv saved")

        #save csvs for key datasets
        arcpy.management.CopyRows( B7Output + "_export" , csvOUT + prefix + "B7_case" + exCase + "_export.csv" )
        arcpy.management.Delete(csvOUT + prefix + "B7_case" + exCase + "_export.csv.xml")
        arcpy.management.Delete(csvOUT + "schema.ini")
        WriteMsg("B7: Saved csvs ( _export )" )

        # delete memory at end of stage
        arcpy.management.Delete("memory")
        end_time = time.time()
        WriteMsg("B7: Memory cache deleted, END mapRE+ B7 taking " + str(round((end_time - start_time) / 60, 1)) + " minutes.")
    ...
    if startStage <= 8 and endStage >= 8:
        WriteMsg("B8:Starting B8 with " + B3Output )
        start_time = time.time()
       ...
       #                                          "m_locMult" , "m_node" ] , "KEEP_FIELDS")
        ...
        ###SPURS
        #prepare
        tx= arcpy.conversion.ExportFeatures( B4Output + "_spurs" , r"memory\spurs")
        ...
        WriteMsg("B8: Added " + B4Output + "_spurs to projects" )

        ###LANDFALL FOR OFF SPURS
        if Resource == "off":
            #prepare
            tx = arcpy.conversion.ExportFeatures(B4Output + "_landfall", r"memory\landF")
            ...
            arcpy.management.Delete(tx)
            WriteMsg("B8: Added " + B4Output + "_landfall to off projects")
        else:
            projSpur = projSpurP

        ###BULK
        #prepare
        tx = arcpy.conversion.ExportFeatures( B5Output + "_bulk" , r"memory\bulk")
        ...
        arcpy.management.Delete(tx)
        WriteMsg("B8: Added " + B5Output + "_bulk to projects" )

        ###SINK
        #prepare
        tx = arcpy.conversion.ExportFeatures( B6Output + "_sink" , r"memory\sink")
        ...
        arcpy.management.Delete(tx)
        WriteMsg("B8: Added " + B6Output + "_sinks to projects" )

        ###PREPARE EXPORT FOR JOIN and add to PROJ
        tx= arcpy.conversion.ExportFeatures( B7Output + "_export" , r"memory\export")
        ...
        arcpy.management.Delete(tx)
        WriteMsg("B8: Added " + B7Output + "_export to projects" )

        ###LANDFALL FOR OFF EXPORT
        if Resource == "off":
            #prepare
            tx = arcpy.conversion.ExportFeatures(B7Output + "_exportLandfall", r"memory\landF")
            arcpy.management.AddField(tx, "landOID", "LONG")
            arcpy.management.AddField(tx, "OIDcom", "LONG")
            ...
            arcpy.management.Delete(tx)
            WriteMsg("B8: Added " + B7Output + "_landfall to off projects")
        else:
            projWall = projWallp

        arcpy.conversion.ExportFeatures(projWall, B8Output )

        ###EXPORT TO RESULTS DIR
        arcpy.management.CopyRows(projWall, csvOUT + prefix + "B8_case" + exCase + ".csv")
        arcpy.management.Delete( csvOUT + prefix + "B8_case" + exCase + ".csv.xml")
        arcpy.management.Delete( csvOUT + "schema.ini")
        ...
        arcpy.management.Delete("memory")
        end_time = time.time()
        WriteMsg("B8: Memory cache deleted, END mapRE+ B8 taking " + str(round((end_time - start_time) / 60, 1)) + " minutes.")
    ...
    if startStage <= 9 and endStage >= 9:
        WriteMsg("B9:Starting B9 with " + B8Output )
        start_time = time.time()

        # delete memory at end of stage
        arcpy.management.Delete("memory")
        end_time = time.time()
        WriteMsg("B9: Memory cache deleted, END mapRE+ B9 taking " + str(round((end_time - start_time) / 60, 1)) + " minutes.")
    ...
    # str(arcpy.GetCount_management(B8Output)) +
    WriteMsg("Column Run: Memory cache deleted, END mapRE+ Column Run for column " + str(column) + " taking " + str(round((ColEnd_time - ColStart_time) / 60, 1)) + " minutes on tasks for " + " " + Resource + " projects.")
    logfile.close()
# _______________________________________________________________________________________________________________________#
##FINISH COLUMN RUN
# delete memory at end of stage
