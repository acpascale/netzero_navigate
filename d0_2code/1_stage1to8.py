...
# -*- coding: utf-8 -*-
"""
Created on Fri Jul 23 11:17:10 2021 -- last NZAU edit 30 August 2022, NZx 29/11/2023 -- last paper edit 26 August 2024
...
mastercsv_ = pd.read_csv( scriptpath + "\\parameters\\" + "mvp0_paper_setup.csv" , header=None)
...
for column in mastercsv:
    ...
    S1Output       = ScratchGDB + "\\_" + prefix + "B1" + "_case" + exCase
    ifTrue         = 1
    ifFalse        = 0
    # STAGE 2 variables #
    ...
    #FishnetDir     = RootDir
    S2Output       = ScratchGDB + "\\_" + prefix + "B2" + "_case" + exCase
    # STAGE 3 variables #
    Stage3CSVInput = scriptpath + data[29]
    S3Output       = OutputGDB + "\\_" + prefix + "B3"  + "_case" + exCase
    ...
    S4Output       = OutputGDB + "\\_" + prefix + "B4"  + "_case" + exCase
    routeBuf       = data[44]
    ...
    S5Output       = OutputGDB + "\\_" + prefix + "B5"  + "_case" + exCase
    bulkField      = data[49]
    ...
    S6Output       = OutputGDB + "\\_" + prefix + "B6"  + "_case" + exCase
    # STAGE 7 variables - Create spur TX Lines from cpa to nearest export node #
    S7Output       = OutputGDB + "\\_" + prefix + "B7"  + "_case" + exCase
    # STAGE 8 variables - Combine all infrastrcture to project#
    S8Output       = OutputGDB + "\\_" + prefix + "B8"  + "_case" + exCase

    #set up logging -- a bit clumsy, should pass logfile name as well... should also add  date
    # from https://community.esri.com/t5/geoprocessing-questions/write-all-add-messages-to-text-file/td-p/1013387
    logfile = open(logpath + prefix + "case" + exCase + "_Newlog.txt", 'a' )
    def WriteMsg(strMsg):
    ...
    arcpy.env.outputCoordinateSystem = TemplateRaster #arcpy.SpatialReference(3577)
    ...
    if redoSubs == 1:
        ...
        arcpy.conversion.ExportFeatures( subsSwww , destSub )
    ...
    if redoSurf_spur == 1:
        ...
        Dir = ( gdbTX + "\\" + "spurDir" )
        Dis = ( gdbTX + "\\" + "spurDis" )
        DistAcc = DistanceAccumulation(gdbTX, in_cost_raster=routeSurf, in_surface_raster=elevSurf, out_back_direction_raster=Dir)
        DistAcc.save(Dis)
    ...
    if startStage <= 4 and endStage >= 4:
        ...
        subs = arcpy.conversion.ExportFeatures(destSub, r"memory\subsClean")
        if destSub == (gispath + "\\netNav_baseAdd.gdb\\GA_2021_Foundation_TXsubstations_3577"):
        ...
        elif destSub == ( gispath + "netNav_base.gdb\\subs"):
        ...
        DistAcc = DistanceAccumulation(destSub, in_cost_raster=constCost, in_surface_raster=elevSurf, out_back_direction_raster=r"memory\SCdir_temp")
    ...
    if startStage <= 8 and endStage >= 8:
       ...
       #                                          "m_locMult" , "m_node" ] , "KEEP_FIELDS")
        ...
        #arcpy.conversion.ExportFeatures(projSpurP, ScratchGDB + "\\A")
        arcpy.management.DeleteField(projSpurP, ["OBJECTID_1"] )
        ...
        if Resource == "off":
            ...
            arcpy.management.RemoveJoin(join)
            arcpy.management.DeleteField(projSpur, ["OBJECTID_1"])
        ...
        arcpy.management.RemoveJoin(join)
        arcpy.management.DeleteField(projSpurBulk, ["OBJECTID_1", "subOID_1"] )
        ...
        arcpy.management.RemoveJoin(join)
        arcpy.management.DeleteField(projSpurBulkSink, ["OBJECTID_1", "subOID_1" , "Shape_Length_1"] )
        ...
        arcpy.management.RemoveJoin(join)
        arcpy.management.DeleteField(projWallp, ["OBJECTID_1", "OIDcom_1" , "OIDcom_12" , "Shape_Length_1" , "Shape_Length_12" , "Shape_Length_12_13"] )
        ...
        if Resource == "off":
            ...
            arcpy.management.RemoveJoin(join)
            arcpy.management.DeleteField(projWall, ["OBJECTID_1"])
    ...
    #_______________________________________________________________________________________________________________________#
    # STAGE 9 -  SKIP ALL (Placeholder)

    ###FUNCTIONS CALLED and REFERENCES


    if startStage <= 9 and endStage >= 9:
        WriteMsg("No action taken on this column" ) # "S9:Starting S9 with " + S8Output )
        start_time = time.time()
