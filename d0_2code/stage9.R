##   stage9.r  - Take output of stage 8 and 
##
##   Created:       15 Jul 2021
##   Last updated:  25 August 2024
##
##    25/08/2024: Clean and prepare for repository
...
wbYes     <- 0  #workbook output? 1 = yes

outVer    <- paste ( "v1" , "_paper" , sep = "" ) 

inVers <- c ( "pv_v0_all_pd45d0_S8_case0"    , "pv_v0_all_pd45d0_S8_case1"   ,  "pv_v0_all_pd45d0_S8_case2"   , "pv_v0_all_pd45d0_S8_case3"   , 
              "wind_v0_all_pd2d7_S8_case0"   , "wind_v0_all_pd2d7_S8_case1"  ,  "wind_v0_all_pd2d7_S8_case2"  , "wind_v0_all_pd2d7_S8_case3"  ,
              "off_v0_all_pd4d4_S8_case0"    , "off_v0_all_pd4d4_S8_case1"   ,  "off_v0_all_pd4d4_S8_case2"   , "off_v0_all_pd4d4_S8_case3"   )
...
#ISP adjustments
ADJ           <- read.csv ( paste ( "../d0_1source/" , "2021-09-28-NZAu_TXmodel_wMults_paper.csv" , sep = ""  ) , header=TRUE , stringsAsFactors = FALSE , fileEncoding = "UTF-8-BOM"  )
...
for (v in 1:( length ( inVers ) ) ) {
  ...
  CPA                <- read.csv  ( paste ( "../d0_3results/cpa_raw/" , inVer , ".csv" , sep = "" ) , header = TRUE , stringsAsFactors = FALSE , fileEncoding = "UTF-8-BOM" )
  ...
  CPAg$m_capOff_maj    <- mean ( subset ( TCC$value , grepl ( "offshore" , TCC$name ) & ( TCC$source == "NREL 2021 ATB" ) & ( TCC$vintage == lcoe.year ) & ( TCC$sensitivity == "ATB2021 Moderate Avg all [d]") ) )
  ...
  #NZAu names
  GEO           <- read.csv ( paste ( "../d0_1source/nzau_geographies_ap.csv" , sep = ""  ) , header=TRUE , stringsAsFactors = FALSE , fileEncoding = "UTF-8-BOM"  )
  GEO           <- GEO[,2:3]
  ...
  ##prepare gis
  LOADS                <- read.csv  ( paste ( "../d0_1source/nzau_destLoad.csv" , sep = "" ) , header = TRUE , stringsAsFactors = FALSE ) 
  ...
  LODE                <- read.csv  ( paste ( "../d0_1source/nzau_destLoad.csv" , sep = "" ) , header = TRUE , stringsAsFactors = FALSE  , fileEncoding = "UTF-8-BOM") 
  ...
  write.csv ( ALLs , paste ( "../d0_3results/sc_raw/_" , resource ,  "Supply_" , outVer , "_case" , case , "_" , CapYear , "_forSC_wBugfix.csv" , sep = "" ) , row.names = FALSE )
